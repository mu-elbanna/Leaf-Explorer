package com.leaf.explorer.fragments.explorer.adapters;
...
import com.leaf.explorer.file_share.util.MimeIconUtils;
import com.leaf.explorer.R;
import com.leaf.explorer.file_share.model.FileShortcutObject;
import com.leaf.explorer.file_share.model.TransferGroup;
import com.leaf.explorer.file_share.model.TransferObject;
...
public class StorageFileAdapter
        extends GroupEditableListAdapter<StorageFileAdapter.GenericFileHolder, GroupEditableListAdapter.GroupViewHolder>
        implements GroupEditableListAdapter.GroupLister.CustomGroupLister<StorageFileAdapter.GenericFileHolder>
{
    ...
    public StorageFileAdapter(Context context)
    {
        ...
        if (path != null) {
            ...
            if (fileIndex != null && fileIndex.length > 0) {
                for (DocumentFile file : fileIndex) {
                    ...
                    SharedPreferences prefs = getContext().getSharedPreferences(com.leaf.explorer.config.AppConfig.SHOW_HIDDEN, 0);
            ...
            List<File> referencedDirectoryList = new ArrayList<>(Arrays.asList(ContextCompat.getExternalFilesDirs(getContext(),null)));
    ...
    public GroupViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType)
    {
        if (viewType == VIEW_TYPE_REPRESENTATIVE)
            return new GroupViewHolder(getInflater().inflate(R.layout.storage_view_title, parent, false), R.id.layout_list_title_text);
        if (viewType == VIEW_TYPE_ACTION_BUTTON)
            return new GroupViewHolder(getInflater().inflate(R.layout.storage_view_button, parent, false), R.id.text);

        return new GroupViewHolder(getInflater().inflate(isGridLayoutRequested()
                ? R.layout.storage_row_file_grid
                : R.layout.storage_row_file, parent, false));

       // return new GroupViewHolder(getInflater().inflate(R.layout.storage_row_file, parent, false));
    }
    ...
    public static class GenericFileHolder extends GroupEditableListAdapter.GroupShareable
    {
        ...
        {
            this(StorageFileAdapter.VIEW_TYPE_REPRESENTATIVE, representativeText);
        }
    ...
    {
        public StorageHolder(Context mContext, File file, String info, int iconRes)
        {
            ...
            this.info = mContext.getResources().getQuantityString(R.plurals.text_files, fileCount, fileCount);
        }
    ...
    public static class RecentFileHolder extends FileHolder
    {
