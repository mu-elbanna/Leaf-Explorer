package com.leaf.music.services
...
import com.leaf.music.util.Constants.NOTIFICATION_ID
import com.leaf.music.util.Constants.POSITION_KEY
import com.leaf.music.util.Constants.PREF_NAME
import com.leaf.music.util.PlayPauseStateNotifier
import com.leaf.music.util.SeekCompletionNotifier
...
class PlayerService : Service(), MediaPlayer.OnCompletionListener, MediaPlayer.OnPreparedListener,
    ...
    private val songFromSharedPreferences: Song?
        get() = SharedPreferenceUtil.getCurrentSong(this, sharedPreferences)
    ...
    override fun onCreate() {
        ...
        if (currentSong == null) {
            currentSong = SharedPreferenceUtil.getCurrentSong(this, sharedPreferences)
        }
    }
    ...
    override fun onAudioFocusChange(focusState: Int) {
        when (focusState) {
            AudioManager.AUDIOFOCUS_GAIN -> {
                if (mediaPlayer == null) {
                    initMediaPlayer(songFromSharedPreferences!!.id)
                } else if (!isPlaying()) {
                    play()
                }
    ...
    private fun getTitle(): Spanned? {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Html.fromHtml("<b>" + currentSong!!.title + "</b>", Html.FROM_HTML_MODE_LEGACY)
        } else {
            Html.fromHtml("<b>" + currentSong!!.title + "</b>")
        }
    }

    private fun getSubText(): Spanned? {
        val subText = if (currentSong?.album != null) currentSong?.album else ""
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
    ...
    private fun startForegroundService() {
        ...
        if (bitmap == null) {
            bitmap = ContextCompat.getDrawable(this, R.drawable.album_art)?.toBitmap()
        }

        var playPauseDrawable = R.drawable.ic_pause_white_24dp
        if (mediaPlayer != null)
        ...
        val builder = androidx.core.app.NotificationCompat
            .Builder(this, NOTIFICATION_CHANNEL_ID).setOngoing(true).apply {
                ...
                addAction(R.drawable.ic_controls_prev, "Previous", previousPendingIntent)
                addAction(playPauseDrawable, "Play", playPausePendingIntent)
                addAction(R.drawable.ic_controls_next, "Next", nextPendingIntent)
                setLargeIcon(bitmap)
