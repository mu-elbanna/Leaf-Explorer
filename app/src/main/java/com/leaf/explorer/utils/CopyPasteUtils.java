...
import com.leaf.explorer.app.SuperUser;
...
public class CopyPasteUtils {
                    ...
                    try {
                        ...
                        synchronized (lock) {
                            if (is != null && os != null) {
                                ...
                                if (diff >= 1000)
                                    e = JavaUtils.formatLeftExact(context, diff);
                                else
                                    e = "∞";
                                title.setGravity(Gravity.CENTER);
                                title.setText(n + " " + JavaUtils.formatSize(context, a) + context.getString(R.string.per_second) + ", " + e);
                                update(this, old, f);
    ...
    public static class OperationBuilder extends MaterialAlertDialogBuilder {
        ...
        public void update(PendingOperation op) {
            ...
            filesTotal.setText(JavaUtils.formatSize(getContext(), op.processed) + " / " + JavaUtils.formatSize(getContext(), op.total));
        }
    ...
    public static AlertDialog pasteError(final OperationBuilder paste, final PendingOperation op, final Throwable e, final boolean move) {
        Log.e(TAG, "paste", e);
        AlertDialog.Builder builder = new MaterialAlertDialogBuilder(paste.getContext());
