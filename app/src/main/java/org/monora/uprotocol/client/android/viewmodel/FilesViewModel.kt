...
class FilesViewModel @Inject internal constructor(
    ...
    val files = Transformations.map(
        liveData {
            requestPath(fileRepository.internalDirectory)
            emitSource(_files)
        }
    ) {
    ...
    val filePicker = Transformations.map(
        liveData {
    ...
    fun renameFileModel(renamePath: DocumentFile, displayName: String): Boolean {
        val context = context.get() ?: return false

        if (renamePath.renameTo(context, displayName) != null) {
            return true
        }
        return false
    }
    ...
    private fun showHidden() : Boolean {
        ...
        val prefs = Activities.getLeafSharedPreferences(mContext, AppConfig.SHOW_HIDDEN)
        return prefs.getBoolean(AppConfig.SHOW_HIDDEN, false)
    }
    ...
    fun getAvailableStorage() = fileRepository.availableStorage
    ...
    var storage = fileRepository.getStorage()
}
