...
class TransferHistoryFragment : Fragment(R.layout.layout_transfer_history) {
    ...
    private val deleteData = ArrayList<ListItem>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.transfers.observe(viewLifecycleOwner) {
            ...
            it.forEachIndexed { _, listItem ->
                deleteData.add(listItem)
            }
        }
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        inflater.inflate(R.menu.transfer_frag_menu, menu)
    }
                    ...
                    .setPositiveButton(R.string.delete_all) { _, _ ->
                        try {
                            for (data in deleteData) {
                                if (data is TransferDetail) viewModel.deleteTransferHistory(data)
                                if (data is SharedText) viewModel.deleteTransferHistory(data)
                                if (data is WebTransfer) viewModel.deleteTransferHistory(data)
                            }
                        } catch (e: Exception) {
